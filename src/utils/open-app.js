...
export default function openApp(flag = true) {
  ...
  if (client.indexOf('wechat') != -1) {
    ...
    openAppScheme(
      ...
      2000,
      () => flag && window.location.hash !== "#/download" && window.location.hash !== "#/wechat" && (window.location.href = `/#/download`)
    );
  } else if (client.indexOf('ios') != -1) {
    openAppScheme(`wenliao://id=${id}&tag=${type}`, 2000, () => flag && window.location.hash !== "#/download" && window.location.hash !== "#/wechat" && (window.location.href = `/#/download`));
  }
}
